...
✅ **Resolved 2026-05-08** — fonts now self-hosted under `assets/fonts/`. [scripts/fetch-fonts.mjs](scripts/fetch-fonts.mjs) downloads the woff2 files (latin subset only) for Cormorant Garamond 400/500/600/700 and Manrope 400/500/600/700/800 from Google Fonts and writes a generated `assets/fonts/fonts.css` with `font-display: swap` `@font-face` blocks pointing at the local files. 9 faces, 268 KB total. Every HTML page (10 root + 189 prerendered PDPs) now loads `assets/fonts/fonts.css` instead of the Google Fonts `<link>` — drops two DNS lookups (fonts.googleapis.com + fonts.gstatic.com) and a cross-origin font request. Run `npm run fetch:fonts` to refresh.
...
✅ **Resolved 2026-05-08** — `wireMobileDrawer()` in [site.js](assets/js/site.js) now caches the drawer's focusables at wire time and adds a `keydown` listener (`trapTab`) on open that cycles Tab/Shift+Tab within the drawer. Listener is removed on close so the rest of the page tabs normally. Combined with the existing Escape-to-close, focus-return-to-toggle on close, and `aria-expanded` / `aria-hidden` toggling, the drawer is now a fully-keyboard-navigable modal dialog.
...
11. ~~**Add Privacy Policy + Terms pages.**~~ ✅ **Done 2026-05-07** — [privacy.html](privacy.html) + [terms.html](terms.html). See C5. ~~⚠️ Owner review recommended for legal-entity / registration details.~~ ✅ Legal entity "K-LINE Design Ltd, trading as K-LINE MEN" added 2026-05-08.
...
19. ~~Self-host fonts.~~ ✅ Done 2026-05-08 — see F4. [F4]
...
25. ~~Implement focus-trap in mobile drawer + skip link.~~ ✅ Done 2026-05-08 (skip link 2026-05-07; focus-trap 2026-05-08). [H6, H7]
