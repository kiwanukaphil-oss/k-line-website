...
- **Logo as favicon.** [index.html:17](index.html#L17) ships a **755 KB** favicon on every page load. Browsers block render until the favicon resolves. Generate a 32×32 / 48×48 / 180×180 set. *(Still open — item #2 in priority plan.)*
- **Footer logo too.** [site.js:263](assets/js/site.js#L263) re-loads the same 755 KB on every page. *(Still open — item #3 in priority plan.)*
...
### F2. Favicon set
Replace the 755 KB `Logo.png` favicon with:
- `favicon.ico` (32×32, ~5 KB)
- `apple-touch-icon.png` (180×180)
- `icon-192.png`, `icon-512.png` for PWA / Android home screen
- A `webmanifest` (optional, but trivial).

### F3. Footer logo
Replace the 755 KB PNG with a 100×100 `Logo-mark.svg` (vector — scales perfectly, ~5 KB). Removes the `mix-blend-mode` hack too.
...
2. **Generate proper favicon set.** [C1, F2] — 30 minutes with realfavicongenerator.net.
3. **Replace footer logo with SVG.** [F3] — drop the `mix-blend-mode` hack.
